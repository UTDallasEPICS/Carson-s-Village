...
<template>
  <CVContainer>
    <form class="p-3 rounded bg-gray-50">
      ...
      <TitleComp v-if="page.id != 0">
        Edit Family Page
      </TitleComp>
      <TitleComp v-else>
        Insert New Family Page
      </TitleComp>
      <br>
    </form>
    <br>
    <div>
      <div class="information rounded-md mx-9 my-2 text-center sm:text-start text-white bg-blue-999">
        <CVLegend>Personal Information</CVLegend>
      </div>
      ...
      <div class="py-4 grid sm:grid-cols-3">
        <CVLabel for="last_name">
          Last Name
        </CVLabel>
        <div class="mx-9 sm:col-span-2 sm:mr-11">
          <CVInput
            id="last_name"
            v-model="page.page_last_name"
            text="text"
            placeholder="required"
            required="required"
          />
          ...
          >
            <div class="relative">
              ...
              <ListboxButton class="text-left bg-white relative rounded-md pl-2 pr-10 py-2 sm:text-sm w-96">
                {{ familyCuid ? currentFamily.family_name : 'Select family to add the page to' }}
              </ListboxButton>
            </div>
          </Listbox>
        </div>
      ...
      <ImagePreview
        v-model:images="page.Images"
        :page-cuid="page.id"
      />
      <div class="information rounded-md mx-9 my-2 text-center sm:text-start text-white bg-blue-999">
        <legend class="ml-4 sm:py-1 font-bold text-shadow-[3px_3px_4px_rgba(0,0,0,0.25)]">
          Profile Image Selection
        </legend>
      </div>
      <div class="py-4 grid sm:grid-cols-3">
        <div class="flex">
          <CVLabel for="profile_image">
            Profile Image
          </CVLabel>
          <CVHelpButton
            class="inline-block z-20"
        ...
        <div class="mx-9 sm:col-span-2 sm:mr-11">
          <Listbox
            id="profile_image"
            v-model="page.profileImageCuid"
            as="div"
            class="rounded-lg outline-0 w-full bg-white p-2"
          >
            <div class="relative">
              <Transition
                leave-active-class="transition ease-in duration-300"
              ...
              <ListboxOptions
                as="div"
                :style="{ 'margin-top': profileImgHeight + 'px' }"
                class="w-full absolute z-10 bg-white shadow-lg shrink-0 max-h-60 max-w-96 mt-8 px-2 py-2 text-base ring-1 ring-black ring-opacity-5 rounded-md overflow-auto focus:outline-none sm:text-sm"
              >
                <ListboxOption
                  v-for="(image,k) in page.Images"
                  :key="image.id"
                  :value="image.id"
                >
                  <img
              ...
              <ListboxButton
                ...
                class="relative rounded-md pl-2 py-4 pr-6 max-w-96 sm:text-sm border border-[#c4c4c4]"
              >
                <div class="flex">
                  <img
                    v-if="profileImage"
                  ...
                  />
                  <CVChevronLeft
                    v-if="toggleFiddlyBit"
                    class="inline-block size-4 h-2 max-w-8 h-4 text-grey-500 z-3"
                  />
                  <CVChevronDown
                    v-else
                    class="inline-block size-4 h-2 max-w-8 h-4 text-gray-500 z-3"
                  />
                </div>
              </ListboxButton>
            </div>
      ...
      <div class="information rounded-md mx-9 my-2 text-center sm:text-start text-white bg-blue-999">
        <CVLegend>Visitation Information</CVLegend>
      </div>
      <div class="py-4 grid sm:grid-cols-3">
        <CVLabel for="visitation_date">
          Date
        </CVLabel>
        <div class="mx-9 sm:col-span-2 sm:mr-11">
          <CVDatepicker
            id="visitation_date"
            v-model="page.visitation_date"
          />
        </div>
      </div>
      <div class="py-4 grid sm:grid-cols-3">
        <CVLabel for="visitation_location">
          Location
        </CVLabel>
        <div class="mx-9 sm:col-span-2 sm:mr-11">
          <CVInput
            id="visitation_location"
            v-model="page.visitation_location"
            placeholder="optional"
          />
        </div>
      </div>
      <div class="py-4 grid sm:grid-cols-3">
        <CVLabel for="visitation_address">
          Address
        </CVLabel>
        <div class="mx-9 sm:col-span-2 sm:mr-11">
          <CVInput
            id="visitation_address"
            v-model="page.visitation_address"
            placeholder="optional"
          />
        </div>
      </div>
      <div class="py-4 grid sm:grid-cols-3">
        <CVLabel for="visitation_description">
          Description
        </CVLabel>
        <div class="mx-9 sm:col-span-2 sm:mr-11">
          <CVTextArea
            id="visitation_description"
            v-model="page.visitation_description"
            placeholder="optional"
          />
        </div>
      </div>
      <div class="information rounded-md mx-9 my-2 text-center sm:text-start text-white bg-blue-999">
        <CVLegend>Funeral Information</CVLegend>
      </div>
      <div class="py-4 grid sm:grid-cols-3">
        <CVLabel for="funeral_date">
          Date
        </CVLabel>
        <div class="mx-9 sm:col-span-2 sm:mr-11">
          <CVDatepicker
            id="funeral_date"
            v-model="page.funeral_date"
          />
        </div>
      </div>
      <div class="py-4 grid sm:grid-cols-3">
        <CVLabel for="funeral_location">
          Location
        </CVLabel>
        <div class="mx-9 sm:col-span-2 sm:mr-11">
          <CVInput
            id="funeral_location"
            v-model="page.funeral_location"
            placeholder="optional"
          />
        </div>
      </div>
      <div class="py-4 grid sm:grid-cols-3">
        <CVLabel for="funeral_address">
          Address
        </CVLabel>
        <div class="mx-9 sm:col-span-2 sm:mr-11">
          <CVInput
            id="funeral_address"
            v-model="page.funeral_address"
            placeholder="optional"
          />
        </div>
      </div>
      <div class="py-4 grid sm:grid-cols-3">
        <CVLabel for="funeral_description">
          Description
        </CVLabel>
        <div class="mx-9 sm:col-span-2 sm:mr-11">
          <CVTextArea
            id="funeral_description"
            v-model="page.funeral_description"
            placeholder="optional"
          />
        </div>
      </div>
      <div class="py-4 grid sm:grid-cols-3">
        <CVLabel for="obituary">
          Obituary
        </CVLabel>
        <div class="mx-9 sm:col-span-2 sm:mr-11">
          <CVTextArea
            id="obituary"
            v-model="page.obituary"
            placeholder="optional"
          />
        </div>
      </div>
      <div class="information rounded-md mx-9 my-2 text-center sm:text-start text-white bg-blue-999">
        <CVLegend>Fundraising Information</CVLegend>
      </div>
      <div class="py-4 grid sm:grid-cols-3">
        <div class="flex">
          <CVLabel for="donation_goal">
            Goal
          </CVLabel>
          <CVHelpButton
            class="inline-block"
            description="If the Donation Goal is 0, it will be assumed that you do not wish to display your donation progress bar and current amount raised."
          />
        </div>
        <div class="flex mx-9 sm:col-span-2 sm:mr-11">
          <span class="rounded-l-md bg-gray-200 text-lg p-2 text-shadow-[3px_3px_4px_rgba(0,0,0,0.25)] border border-[#c4c4c4]">$</span>
          <input
            id="donation_goal"
            v-model="page.donation_goal"
            class="outline-0 rounded-r-md border-box w-full p-2 border border-[#c4c4c4]"
          >
        </div>
      </div>
      <div class="py-4 grid sm:grid-cols-3">
        <CVLabel for="deadline">
          Deadline Date
        </CVLabel>
        <div class="mx-9 sm:col-span-2 sm:mr-11">
          <CVDatepicker
            id="deadline"
            v-model="page.deadline"
          />
        </div>
      </div>
      <div class="py-4 grid sm:grid-cols-3">
        <CVLabel for="donation_description">
          Description
        </CVLabel>
        <div class="mx-9 sm:col-span-2 sm:mr-11">
          <CVTextArea
            id="donation_description"
            v-model="page.donation_description"
            placeholder="optional"
          />
        </div>
      </div>
      ...
      >
        <CVLegend>Comment Moderation</CVLegend>
      </div>
      <div
        v-if="replies?.length"
        class="py-4 grid flex-box flex-row item-centered gap-1 leading-none"
      >
        <div class="flex" />
        <div
        ...
        >
          <CVReply :r="reply" />
          <CVSuspendButton
            :suspend="reply.suspended"
            :rep="reply"
            @update:suspend="updateSuspendButton"
          />
        </div>
      </div>
      <div class="ml-9 mb-9 py-7 flex flex-wrap gap-2">
        <div class="px-2 mt-2">
          <ActionButton
            ...
            class="sm:my-2 transition duration-300 bg-orange-999 hover:bg-green-600 disabled:bg-orange-800 disabled:cursor-not-allowed"
            @click="save"
          />
        </div>
        <div class="py-2 mt-2">
      ...
      >
        <CVLabel for="error">
          Error in Creating/Editing page in the system.
        </CVLabel>
      </div>
    </div>
  </CVContainer>
