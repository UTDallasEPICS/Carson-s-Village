...
div
    ...
    br
    div(class="sm:grid-cols-3 py-4 grid" v-if="date_ranged")
        CVLabel Date Field
    ...
    div(class="flex gap-5 align-center justify-center" v-if="date_ranged")
      CVLabel Date Range
      CVDatepicker(v-model='date' @update:model-value="currentPage=0; loadReports();" range)
    h2(class="mt-4 ml-10 border-1 text-[23px] border-black underline") Table Dimensions
      div(class="flex flex-box flex-wrap gap-10 w-[450px]") 
        h1(class="ml-1 text-[18px]") Number of Table Rows
          CVInputNumerical(v-model="dimensions")
    h2(class="mt-4 ml-10 text-[18px] underline") Fields to Show
    div(class="w-[700px] sm:grid-cols-3 flex flex-box flex-wrap gap-10 stretch grid ml-2") 
      label(class="sm:mt-0 mt-4 ml-10 text-md tracking-[0.35px]") Duration
        div(class="w-full")
      ...
      label(class="sm:mt-0 mt-4 ml-10 text-md tracking-[0.35px]") Amount Paid Out
        div
          input(type='checkbox' v-model="display.paid")
      label(class="sm:mt-0 mt-4 ml-10 text-md tracking-[0.35px]") Goal Date
        div
