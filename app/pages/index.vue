...
<template>
  <div class=" container overflow-hidden mt-4 mx-auto">
    ...
    <div class="flex flex-col justify-start gap-6 sm:grid sm:gap-10 sm:grid-rows-3 sm:grid-cols-2 sm:my-12">
      <TextGrayField class="sm:mx-auto">
        Name:
      </TextGrayField>
      <TextGrayField>{{ user?.name }}</TextGrayField>
      <TextGrayField class="sm:mx-auto">
        Email:
      </TextGrayField>
      <TextGrayField>{{ user?.email }}</TextGrayField>
      <TextGrayField class="sm:mx-auto">
        Phone:
      </TextGrayField>
      <TextGrayField>{{ formatPhoneForDisplay(user?.phone) }}</TextGrayField>
    </div>
  </div>
  <LinkButton
