...
const id2 = computed(() =>  router.params.id2);
const transaction_id = id2.value as string;
const cuid = id.value as string;
...
html
    ...
    .row.text-center
        .text-gray-dark.font-poppins.text-md.inline-block.text-center {{ formatDate(pageData.day_of_birth) }} 
        .text-gray-dark.font-poppins.text-md.inline-block.whitespace-pre  - 
        .text-gray-dark.font-poppins.text-md.inline-block {{ formatDate(pageData.day_of_passing) }}
    .row.p-3
        NuxtLink.mx-16.p-3.px-6.pt-2.text-white.bg-orange-400.font-poppins(style="font-weight: 700; border-radius: 32px;" :to="`/PageList/${id}`") Back
            ...
            .div.p-4(id="services")
                .div(v-if="pageData.visitation_date")
                    ...
                    .grid.gap-x-3.gap-y-0.grid-cols-5.grid-rows-2
                        img.px-4(:src="`${clock}`")
                        .py-1.font-outfit.text-dark-blue.col-span-4(style="font-size: 20px; line-height: 30px;") {{ formatDate(pageData.visitation_date) }}"
                ...
                .div(v-if="pageData.funeral_date")
                    ...
                    .grid.gap-x-3.gap-y-0.grid-cols-5.grid-rows-2
                        img.px-4(:src="`${clock}`")
                        .py-1.font-outfit.text-dark-blue.col-span-4(style="font-size: 20px; line-height: 30px;") {{ formatDate(pageData.funeral_date) }}
                        img.px-2(:src="`${location}`")
            ...
            .div.p-4(id="donations")
                .container(class="sm:overflow-hidden sm:w-3/4 sm:mt-4 sm:mx-auto sm:place-content-center sm:max-w-xl sm:p-6 sm:rounded-card sm:shadow-card")
                    .container.m-4.place-content-center.font-poppins(class="w-5/6 sm:m-auto sm:py-3")
                        .text-md.text-center.ml-4.my-3(class="sm:text-xl sm:my-6" style="letter-spacing: 0.35px; font-weight: 600; color: #646464;") {{ pageData.amount_raised  + " raised of " +  pageData.donation_goal + " goal" }}
                        ...
                        br
                        .form-horizontal(method='post' action=userAction)
