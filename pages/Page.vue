...
html
	...
	.text-gray-dark.font-poppins.text-2xl.text-center.font-semibold(style="text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.25);") {{data.page_name}}
	.row.text-center
		.text-gray-dark.font-poppins.text-md.inline-block.text-center {{data.day_of_birth}}
		.text-gray-dark.font-poppins.text-md.inline-block.whitespace-pre  - 
		.text-gray-dark.font-poppins.text-md.inline-block {{data.day_of_passing}}
	.row.p-3
	...
	.container.bg-gray-light.mx-auto.h-auto.font-poppins(class="w-11/12 rounded-[8px]" style="box-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25);")
		...
		.div(style="background: #F8F8F8;" id="tab-contents")
			.div.px-8.py-4(style="color: #6E6E6E; font-weight: 500; font-size: 14px; line-height: 28px; letter-spacing: -0.078px; word-break: break-word;" id="obituary") {{data.obituary}} Obituary 
			.div.p-4(id="services")
				if visitation_date
					...
					.px-5.pt-2.pb-8.font-outfit.text-dark-blue(style="font-size: 20px; line-height: 30px;") {{data.visation_description}}
					.grid.gap-x-3.gap-y-0.grid-cols-5.grid-rows-2
						img.px-4(src="/images/clock_icon.png")
						.py-1.font-outfit.text-dark-blue.col-span-4(style="font-size: 20px; line-height: 30px;") {{data.visitation_date + ", " + data.timezone}}
				...
				if funeral_date
					...
					.px-5.pt-2.pb-8.font-outfit.text-dark-blue(style="font-size: 20px; line-height: 30px;") {{data.funeral_description}}
					.grid.gap-x-3.gap-y-0.grid-cols-5.grid-rows-2
						img.px-4(src="/images/clock_icon.png")
						.py-1.font-outfit.text-dark-blue.col-span-4(style="font-size: 20px; line-height: 30px;") {{data.funeral_date + ", " + data.timezone}}
						img.px-2(src="/images/location_icon.png")
			...
			.div.p-4(id="donations")
				.container(class="sm:overflow-hidden sm:w-3/4 sm:mt-4 sm:mx-auto sm:place-content-center sm:max-w-xl sm:p-6 sm:rounded-card sm:shadow-card")
					.container.m-4.place-content-center.font-poppins(class="w-5/6 sm:m-auto sm:py-3")
						.text-md.text-center.ml-4.my-3(class="sm:text-xl sm:my-6" style="letter-spacing: 0.35px; font-weight: 600; color: #646464;") {{data.donated_amount + " raised of " + data.donation_goal + " goal"}}
						.progress-bar.overflow-hidden.ml-4.h-5.rounded-full(style="30px; background-color:#b5b5b5;")
							.progress.rounded-full.text-white.flex.items-center.justify-center(donated-amount=donated_percentage style="background: linear-gradient(90deg, rgba(15,200,0,1) 35%, rgba(203,255,0,1) 100%); box-shadow: 0 3px 3px -5px #1ba710, 0 2px 5px #1ba710; height: 100%; opacity: 0; width:0px;") {{data.donated_percentage + "%"}}
						.well.well-sm
						...
						form.form-horizontal(method='post' action=userAction)
							fieldset
								.col-md-8.ml-4.pt-1.pr-5(class="sm:mx-4 sm:w-full sm:py-2")
									input#first_name.rounded-md.outline-0.border-box.w-full.p-2(style="border: 1px solid #c4c4c4;" v-model='first_name' type='text' placeholder='First Name' required)
								.col-md-8.ml-4.pt-1.pr-5(class="sm:mx-4 sm:w-full sm:py-2")
									input#last_name.rounded-md.outline-0.border-box.w-full.p-2(style="border: 1px solid #c4c4c4;" v-model='last_name' type='text' placeholder='Last Name' required)
								...
								.col-md-8.ml-4.pt-4.pr-5.flex(class="sm:mx-4 sm:w-full sm:py-2")
									textarea#comments.rounded-md.outline-0.border-box.w-full.p-2(style="border: 1px solid #c4c4c4;" v-model='comments' rows='3' placeholder='Comments' required)
								.col-md-8.ml-4.pt-4.pr-5.grid.grid-cols-3(class="sm:mx-4 sm:w-full sm:py-2")
									...
									.flex
										...
										input#donation_amount.bg-gray-light.outline-0.rounded-r-md.border-box.w-full.p-2(style="border: 1px solid #c4c4c4; border-left:none;" v-model='donation_amount' type="number" min="0.00" step="0.01" required)
