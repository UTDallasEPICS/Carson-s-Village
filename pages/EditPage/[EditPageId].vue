...
const getImages = async (pageCuid: string) => {
    ...
    //await getImages(useRoute().params.EditPageId as string)
/*onMounted(async() => {
if (cuid !== "0")
  await getData();
})*/
        ...
        .py-4.grid.flex-box.flex-directional-row.item-centered.gap-1(v-if="imageData.length!= 0" class="sm:grid-cols-3" style="line-height: 0px;text-align: center")
            .div(style='position: relative;') 
                ...
                .form-horizontal(style='position: absolute; top: 5px; right: 5px')
                    button#remove.bg-red-500(style="display: flex;align-items: center;justify-content: center;line-height: 0px;text-align: center; color: white; font-weight: 450; positon: absolute; top:0px; left: 0px; width: 30px; height: 30px; border-radius: 50%;" @click = "removeImage(selectedImageObj)") x
