...
form
    ...
    .col-md-8.ml-4.pt-4.pr-5.flex(class="sm:mx-4 sm:w-full sm:py-2")
        textarea#comments.rounded-md.outline-0.border-box.w-full.p-2(style="border: 1px solid #c4c4c4;" name='comments' rows="3" v-model="donationData.comments" placeholder='Comments')
    ...
    label.mt-4.ml-4.text-md(for='fee_recovery' class="sm:mt-0" style="letter-spacing: 0.35px;") I'd like to help cover the transaction fees of ${{ props.isActive ? (0.029 * donationData.amount + 0.30).toFixed(2) : 0}} for my donation. 
    .col-md-8.ml-4.pt-6.pr-5.flex.items-center.justify-center
        ActionButton.mx-auto.text-md(@click="create_checkout_session" :disabled="donationData.amount < 5" :class="{'transition duration-300 bg-orange-999 hover:bg-green-600': true, 'cursor-not-allowed': donationData.amount < 5 }") DONATE NOW
</template>
